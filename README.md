...
**Fix real tras seguir insistiendo el usuario** ("swipe to delete keeps activating/showing when sending a track to temporal list"): con el bug de estado ya descartado y sin lograr reproducir nada anómalo en el `translateX`, la causa real resultó ser de **color, no de lógica** — el aviso de "añadir a la cola" (revelado al deslizar a la derecha, acción no destructiva) usaba el mismo `bg-oxblood` que el botón de borrar (acción destructiva), así que cualquier reveal rojizo al deslizar se leía como "se está activando el borrado" pese a tratarse siempre del lado izquierdo/añadir. Cambiado a `bg-moss` (el mismo verde que ya usan los toasts de éxito en `Toast.tsx` — contraste verificado ≥7:1 con el texto en pergamino), separando visualmente de forma inequívoca la acción positiva (verde) de la destructiva (oxblood). **Patrón a recordar**: cuando dos affordances de un mismo componente comparten familia de color solo porque "quedaba bien", un usuario real puede fundirlas en una sola categoría mental (aquí, "rojo = borrar" absorbiendo también al swipe de añadir) incluso si el código nunca mezcla su lógica — la semántica de color en UI de gestos debe reforzar la distinción funcional, no solo la estética.
