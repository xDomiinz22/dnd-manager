...
### ✅ Gestos de swipe en canciones: cola "Reproducir después" y borrado por deslizar + atajos de teclado

Investigado primero el estado del arte de gestos de swipe en listas (patrón Gmail/iOS Mail: deslizar revela una acción; algunas apps usan además un swipe "hacia el otro lado" para una acción secundaria no destructiva, como "posponer" en Gmail o "reproducir a continuación" en reproductores de música) para diseñar esto de forma consistente con esos patrones ya establecidos, en vez de inventar una interacción nueva.

- **Cola "Reproducir después"** (`AmbientPlayerContext.tsx`): puramente local al navegador (no se guarda en el backend — es una preferencia de escucha efímera, igual de criterio que el volumen o el shuffle). Deslizar un track de izquierda a derecha lo añade al final; `playNext`/el fin natural de un track la vacían por orden (FIFO) **antes** de seguir con la lista normal — mismo comportamiento que la cola "Reproducir a continuación" de Spotify, confirmado con el usuario. Se considera "creada" en cuanto tiene algún elemento y "borrada" en cuanto vuelve a quedar vacía, sin modelar un estado aparte. Mientras suena un track de la cola, el puntero de la lista normal se queda quieto (para retomarla justo donde estaba); "Anterior" con un track de la cola sonando reinicia ese mismo track desde el principio (no hay un "anterior" razonable que reconstruir una vez la cola perdió ese elemento).
- **`SwipeableRow.tsx`** (`components/ui/`): fila con gestos horizontales por Pointer Events (ratón y touch). A la derecha dispara la acción de inmediato (no destructiva, sin confirmación); a la izquierda revela un botón rojo de borrar que hay que pulsar de forma explícita — el propio gesto de revelar + pulsar ya hace de confirmación, sin diálogo encima (decisión tomada con el usuario, comparando con la alternativa de "deslizar más allá de un segundo umbral", más rápida pero más fácil de disparar sin querer). Detecta si el gesto es horizontal o vertical antes de "capturar" el puntero (para no romper el scroll vertical de la página), y respeta `data-no-swipe` en el asa de arrastre de reordenar y en los botones Editar/Mover/Borrar (para no competir con esos gestos/clicks).
- **Backend**: ninguno — toda la cola es estado de React en `AmbientPlayerContext`, sin tocar Prisma ni la API.
- **`TempQueuePanel.tsx` + `TempQueueLauncher.tsx`** (`components/music/`): en escritorio, una pestaña fija al borde derecho (con contador) que despliega un panel lateral; en móvil, un botón flotante (FAB) que abre una hoja desde abajo — ambos comparten el mismo `TempQueuePanel` como contenido, cada fila de la cola también se puede quitar deslizando (mismo `SwipeableRow`). Solo se muestra cuando la cola tiene algo (para no añadir ruido visual al resto del sitio).
- **Atajos de teclado** (`AmbientPlayerContext.tsx`): Espacio pausa/reproduce; ← / → saltan 10s atrás/adelante en el track actual; Mayús+← / Mayús+→ cambian de track anterior/siguiente (sin dar la vuelta al llegar al principio/final de la lista — a propósito distinto de los botones del reproductor, que sí dan la vuelta; con shuffle activo o reproduciendo desde la cola temporal no se bloquea, ya que ahí "primero/último" no está bien definido); ↑ / ↓ suben/bajan el volumen; M silencia/reactiva el sonido. Se ignoran si el foco está en un campo de texto (input/textarea/select/contenteditable), y no hacen nada si no hay ningún track cargado.
- **Gotcha de ESLint, reafirmado**: la regla `react-hooks/exhaustive-deps` de esta instalación crashea (`context.getSource is not a function`) al analizar un `useEffect` que referencia directamente una función de componente no memoizada con un cuerpo complejo — no solo con `useCallback` (gotcha ya documentado antes en esta sesión), también con un `useEffect(() => { document.addEventListener("keydown", handleKeyDown); ... }, [])` normal. Ni el disable inline ni el disable a nivel de archivo evitan el crash (la regla revienta durante su propio análisis, antes de llegar a filtrar el reporte). **Fix real**: pasar la función por un `ref` actualizado en cada render (mismo patrón ya usado para `playRef`/`onEndedRef`) y que el efecto solo llame a `ref.current(e)` — la regla no necesita analizar el cuerpo de una función guardada en un ref.

Verificado en navegador con gestos simulados (secuencia real de `pointerdown/pointermove/pointerup`, ya que `.click()` sintético no sirve para gestos): swipe derecha añade a la cola con toast; swipe izquierda corto (por debajo del umbral) no revela nada; swipe izquierda completo revela el botón sin borrar todavía, y un click fuera lo cierra sin borrar; pulsar el botón revelado sí borra. Prioridad de la cola confirmada: con un track en cola, pulsar "Siguiente" la vacía y reproduce ese track antes de seguir con la lista normal. Los 6 atajos de teclado (espacio, ←/→, Mayús+←/→, ↑/↓, M) verificados uno a uno con `KeyboardEvent` reales, incluyendo que no se disparan si el foco está en el buscador. `typecheck`/`lint`/`test`/`build` limpios.

**Hallazgo menor, no bloqueante**: se observó un warning de React en consola ("Cannot update a component (AmbientPlayerProvider) while rendering a different component (SwipeableRow)") en el caso concreto de que la cola temporal se vacíe (quedándose en 0, lo que desmonta el panel) en el mismo instante en que otra fila de `SwipeableRow` de la lista normal se re-renderiza por cambiar de track activo. Confirmado que no tiene impacto funcional (los datos y la UI quedan siempre correctos en todas las pruebas) y solo aparece en desarrollo (React lo omite en producción). No se ha perseguido más a fondo dado el coste/beneficio.

### ✅ Cuatro fixes sobre los gestos de swipe recién lanzados

El usuario reportó, tras usar la función en real: el deslizamiento a veces se quedaba a medio camino en pantalla; añadir un track a la cola disparaba varios toasts de "añadido" en vez de uno; el track añadido a la cola no llegaba a sonar; y el sombreado de "pulsado" se quedaba pegado en pausa/play, además de que el texto de la pista de swipe-derecha no cabía en su hueco.

- **Toasts duplicados y deslizamiento atascado** (`SwipeableRow.tsx`): la causa real era llamar a los efectos secundarios (`onSwipeRight()`, que dispara el toast) **dentro** de la forma funcional de un `setState` (`setTranslateX((current) => { ...; onSwipeRight(); ... })`) — React StrictMode invoca dos veces esas funciones updater para comprobar que son puras, así que el efecto se disparaba dos veces. Reescrito para que `endGesture()` lea el valor final por un `ref` espejo (`translateXRef`) y llame a los efectos secundarios como sentencias sueltas, nunca desde dentro de un updater. De paso se añadió una red de seguridad de `window.addEventListener("pointerup"/"pointercancel", ...)` (con el patrón de "trampolín" por `ref` para tener una identidad de función estable entre renders, si no `removeEventListener` no encuentra el listener que hay que quitar) para que el gesto siempre termine aunque se pierda la captura del puntero a medio arrastre.
- **El track de la cola no sonaba** (`AmbientPlayerContext.tsx`): cuando no sonaba nada, `addToTempQueue` metía el track en la cola y confiaba en que `advance()` lo sacara enseguida, pero `advance()` leía `tempQueueRef.current` justo después de un `setTempQueue(...)` que aún no se había reflejado en el ref (el espejo solo se actualiza en el siguiente render). `addToTempQueue` ahora comprueba con refs si el reproductor está inactivo y, si lo está, reproduce el track directamente (`setActiveTempTrack` + `playRef.current(...)`) en vez de encolar-y-confiar. Un segundo bug relacionado, más sutil: la mini-barra solo se muestra si `player.groupId` está fijado, y hasta ahora solo lo fijaba `playFromPlaylist`/`playFromPlaylistShuffled` — si el reproductor arrancaba desde cero por un swipe (sin haber pulsado play en una lista antes), `groupId` se quedaba `null` y la mini-barra nunca aparecía aunque el audio sí estuviera cargado. `addToTempQueue` recibe ahora el `groupId` de quien lo llama y lo fija también en la rama de reproducción directa.
- **Sombreado pegado en pausa/play** (`PlayerControls.tsx`): mismo fix ya aplicado a los botones de siguiente/anterior — `e.currentTarget.blur()` en el `onClick`.
- **Texto del hint demasiado grande** (`SwipeableRow.tsx`): etiqueta por defecto acortada a "A la cola", hueco revelado ampliado de 96px a 112px, y tipografía reducida/centrada (`text-[0.7rem]`, `leading-tight`, `text-center`) para que quepa entera.

Verificado en navegador: tras el fix, un swipe-derecha sobre un track con el reproductor inactivo hace aparecer la mini-barra y el contador de tiempo avanza (antes se quedaba en `barExists: false`); solo un toast por swipe (antes salían varios); el hint "A la cola" mide 112px de ancho sin desbordar (`scrollWidth === clientWidth`). `typecheck`/`lint`/`test` (24/24)/`build` limpios.

### ✅ Cola reordenable + robustecer el estado del swipe (cubo de basura atascado)

El usuario pidió que la cola "Reproducir después" fuera editable como una lista normal, y reportó que el cubo de basura del swipe (revelado al deslizar a la izquierda) a veces se quedaba visible de forma permanente incluso al añadir un track (swipe a la derecha).

- **Cola reordenable**: `TempQueuePanel.tsx` ganó `DndContext`/`SortableContext` (mismo patrón `@dnd-kit` que las listas normales), asa de arrastre por fila y `reorderTempQueue(orderedIds)` nuevo en `AmbientPlayerContext.tsx` — reconstruye el array de tracks en el nuevo orden a partir de los ids (no toca el backend, la cola sigue siendo puramente local).
- **`SwipeableRow.tsx` — modelo de estado simplificado a una sola fuente de verdad**: el código anterior guardaba el "lado revelado" en un `ref` aparte (`openSideRef`) además del `translateX` real, y los usaba para cosas distintas (`openSideRef` para decidir la posición base del siguiente arrastre; `translateX` para el CSS) — un vector de desincronización entre ambos no se pudo descartar del todo. Revisando el código se encontró además un bug real y confirmado: al terminar un gesto que no cruza ningún umbral, el código hacía `setOpenSide(null)` y ACTO SEGUIDO comprobaba `openSideRef.current === "left"` para decidir si volver a -72px o a 0 — como `setOpenSide(null)` ya había puesto esa ref a `null` en la línea anterior, esa comprobación era papel mojado: siempre volvía a 0, nunca respetaba que la fila viniera ya abierta. **Fix**: se eliminó `openSideRef` por completo; el gesto ahora guarda su propia posición de reposo (`startTranslateX = translateXRef.current`) al empezar y la usa como base para el arrastre y como destino de "snap back" si no cruza ningún umbral — una sola variable, sin ambigüedad posible entre "lo que se ve" y "lo que el componente cree que se ve".

Verificado en navegador: arrastrar por el asa dentro de la cola reordena los tracks (confirmado con 2 tracks, el orden se invierte tras el drag); quitar un track de la cola (swipe + botón revelado, y también el botón "×") sigue funcionando tras el refactor; `typecheck`/`lint`/`test` (24/24)/`build` limpios. El caso exacto del cubo atascado no se pudo reproducir de forma determinista en este entorno (varios intentos con gestos sintéticos de ratón y touch, con y sin reproducción activa de fondo, siempre resolvieron a 0 correctamente) — el fix aplicado elimina la única inconsistencia de estado real que se encontró revisando el código a fondo.

**Primer intento (color) insuficiente**: cambiar el hint de añadir a verde (`bg-moss`) no lo arregló — el usuario confirmó que seguía pasando, "en móvil y PC", y dio la pista decisiva: **en PC, con solo pasar el ratón por encima de una fila (sin arrastrar ni hacer click) ya se veía el botón de borrar**. Eso descarta cualquier lógica de gesto — apuntaba directamente a CSS.

**Causa real, encontrada con esa pista**: `TrackRow`'s `contentClassName` (el div que se desliza por encima de los "bolsillos" de `SwipeableRow`) usaba fondos **translúcidos**: `hover:bg-parchment-deep/40` (40% opacidad al pasar el ratón) para filas normales, y `bg-oxblood/10` (10% opacidad) para la fila del track que suena. Como el botón de borrar (oxblood-dark, opaco) vive DEBAJO de ese div en el DOM, cualquier opacidad menor que 100% deja que se transparente — con solo el hover (¡sin swipe alguno!) ya se veía, y en la fila del track activo se veía SIEMPRE (10% de opacidad constante, no solo al pasar el ratón). Esto explica también el reporte anterior de "se queda activo tras un swipe de añadir": el cursor/dedo sigue sobre esa fila justo después del gesto, así que el hover se activa inmediatamente después. **Fix**: los dos fondos pasan a ser sólidos (`hover:bg-[#dfcb9b]`, `bg-[#d8c099]` — mismo tono resultante de mezclar el color original con el fondo base al 40%/10%, para que el aspecto visual no cambie, solo la opacidad). **Patrón a recordar, importante para cualquier fila con `SwipeableRow` en este proyecto**: el div de `contentClassName` NUNCA puede llevar un modificador de opacidad Tailwind (`/NN`) en su fondo — tiene que tapar per completo lo que hay debajo en TODO momento (reposo, hover, estado activo), no solo durante el arrastre; cualquier futura fila que use `SwipeableRow` con `onDelete`/`onSwipeRight` debe revisarse por este mismo patrón antes de añadir un hover o un estado "activo" con color.
